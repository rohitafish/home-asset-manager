...
- CI runs `ruff` (lint) and the full test suite on every push and pull
  request, plus two scans as a server-side backstop: the project's own
  PII/secret scanner (see below) and a gitleaks pass over the whole history
  for credential shapes; a monthly TruffleHog workflow additionally verifies
  any candidate against its issuing provider, so a finding there means a
  live credential. GitHub's secret scanning with push protection is on as
  well. Every CI action is pinned to a commit SHA, not a mutable version tag
  — GitHub's `sha_pinning_required` is on for this repo, so an unpinned
  action can't creep back in.
...
PII/secret scanner (`scripts/check-pii.sh`) as a pre-commit hook (on the
index, before a commit exists), a pre-push hook and a CI check, precisely so
that discipline doesn't depend on anyone remembering to apply it by hand.
gitleaks runs beside it at each of those points. The two are not redundant:
a generic secrets scanner recognises credential shapes and nothing else, and
a measured run over this repo's history found none of the real device
identifiers that `check-pii.sh`'s known-values list catches — so the list is
