...
pip-compile** (pip-tools): every direct *and transitive* package, exact
version, with a sha256 hash for each wheel. Don't edit them by hand. The
files you edit are `requirements.in` and `requirements-dev.in`, which hold
only the direct dependencies, exactly pinned. To change one:

```bash
pip install pip-tools                       # once, anywhere (a scratch venv is fine)
...
this layout and regenerates the lock in its own PRs. Installing with
`--require-hashes` (what CI and `scripts/redeploy.sh` do) means a package
whose published file doesn't match the recorded hash is refused rather than
run -- the point of the lock. CI also runs `pip-audit` against the lock on
every push and on a weekly schedule, so a new advisory against a pinned
version shows up without anyone remembering to check.
