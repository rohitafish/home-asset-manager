...
file that installs fine and passes CI -- but its regeneration does **not**
reproduce a true `--universal` resolution: it silently drops platform-only
packages and markers (`colorama`, `tzdata` and `httpx2-jsfetch` on
Windows/emscripten; the `platform_machine`/`sys_platform` markers on
`greenlet`, `httpcore2`, `truststore`, `uvloop` and `psycopg-binary` all
collapsed to unconditional) even on a file whose header still reads
"autogenerated by uv ... --universal". This is exactly the class of
narrowing that broke this app's own CI once already -- the SQLAlchemy
...
guarantee the lock exists to provide. It has happened on every pip PR so
far -- #16-19, then #21 -- so treat it as certain, not occasional.

**Restoring that coverage is automated; it needs nothing from you.** A
scheduled job on the Mini merges Dependabot's PRs once their checks are
green and pushes the regeneration itself, as the
`Restore --universal coverage ...` commits in the log. It re-runs exactly
the command above, and refuses to push a result that moves a version pin,
drops a package, fails a `--require-hashes` install, or fails
`check-pii.sh` -- so a bump needing a human is left open and commented on
rather than merged. That covers the bare-pins-and-no-hashes case too: such
a PR fails CI's own hash-checked install, so it never reaches the merge
gate. The job lives outside this repo (it watches `gmail-labels` as well);
what shows up here is its commits. What it deliberately does not touch is
changing a pin yourself -- that is the workflow above, by hand.

Installing with `--require-hashes` (what CI and `scripts/redeploy.sh` do)
means a package whose published file doesn't match the recorded hash is
refused rather than run -- the point of the lock. CI also runs `pip-audit`
against the lock on every push and on a weekly schedule, so a new advisory
against a pinned version shows up without anyone remembering to check.
