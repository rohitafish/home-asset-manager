...
a `uv`-generated lock (same as it recognised pip-compile's) and regenerates
it in its own PRs; if a Dependabot PR ever comes through with a bare pin and
no hashes, regenerate by hand with the command above before merging. Installing
